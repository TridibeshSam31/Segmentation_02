...
class DualityOffroadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ==========================================
        # DATA AUGMENTATION BLOCK (Training Only)
        # ==========================================
        if self.train:
            # 1. Horizontal Flip Only (Left/Right object appearance)
            if random.random() > 0.5:
                image = TF.hflip(image)
                raw_mask = TF.hflip(raw_mask)
            
            # 2. Affine Transforms (Suspension flex & uneven dune slopes)
            # Tilts the image up to 15 degrees and slightly scales it
            if random.random() > 0.5:
                ...
                # CRITICAL: Mask uses NEAREST so class IDs aren't blurred/blended
                raw_mask = TF.affine(raw_mask, angle=angle, translate=[0,0], scale=scale, shear=0, 
                                     interpolation=TF.InterpolationMode.NEAREST)

            # 3. ColorJitter (Harsh Desert Lighting Simulator)
            # Forces the model to ignore exact sand color and focus on shapes
            if random.random() > 0.5:
            ...
            # 4. Engine Vibration / Dust Simulator
            if random.random() > 0.5:
                image = TF.gaussian_blur(image, kernel_size=[3, 3])
                
        # ==========================================

        # Process the mask into your continuous IDs
        ...
        # Pass through the Hugging Face processor
        ...
        # Safely unpack the processor outputs
        processed_inputs = {}
        ...
        # Save the original un-processed mask for accuracy evaluation later
...
# ==========================================
# NEW: Oversampling Weight Calculator
# ==========================================
def get_oversampling_weights(mask_dir):
    ...
    # Define which raw pixel values are "rare" and how much to boost their chance of being picked
    # 550: Ground Clutter, 700: Logs, 800: Rocks
    rare_multipliers = {
        500: 2.0,
        550: 3.0,  # If image has ground clutter, increase pick chance by 3x
        700: 5.0,  # If image has logs, increase pick chance by 5x
        800: 4.0   # If image has rocks, increase pick chance by 4x
    }
    ...
    for mask_name in tqdm(mask_names, desc="Analyzing Training Masks"):
        ...
        weight = 0.5  # Base weight for a normal image
        
        # Check if the mask contains any of our rare classes
...
def refine_model(train_dir_img, train_dir_mask, val_dir_img, val_dir_mask, epochs=3, batch_size=2):
    ...
    model_path = "best_offroad_model_refined"
    ...
    class_weights = torch.tensor([1.5, 2.0, 2.0, 4.5, 10.0, 2.5, 2.5, 3.5, 1.5, 0.1]).to(device)
    ...
    # replacement=True allows the same "Log" image to be pulled multiple times in one epoch
